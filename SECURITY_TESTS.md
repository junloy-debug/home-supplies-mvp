...
members share one inventory, while everyone else sees nothing.
...
| As pending, select `records` or another member's row | Returns no rows. | `approved members read inventory` requires manager, clerk, or viewer; pending has none. `members` also has no pending-user SELECT policy. |
...
2. Sign up as a second account: use that session to confirm both `members` and
   `records` selects return no data and record inserts fail.
